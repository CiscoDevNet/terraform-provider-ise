...
func NetworkAccessConditionHashChildrenChildren(child NetworkAccessConditionChildrenChildren) string {
...
func NetworkAccessConditionHashChildren(child NetworkAccessConditionChildren) string {
	// Hash the current child's fields
	parts := []string{
		child.Name.ValueString(),
	...
	for _, nestedChild := range child.Children {
		nestedHashes = append(nestedHashes, NetworkAccessConditionHashChildrenChildren(nestedChild))
	}
...
func NetworkAccessConditionFindMatchingChild(res gjson.Result, child NetworkAccessConditionChildren) []gjson.Result {
	...
	res.ForEach(func(_, v gjson.Result) bool {
		...
		var nestedChildren []NetworkAccessConditionChildrenChildren
		jsonChild.Get("children").ForEach(func(_, vChild gjson.Result) bool {
			nestedChild := NetworkAccessConditionChildrenChildren{
		...
		jsonChildHash := NetworkAccessConditionHashChildren(jsonChildStruct)
...
func NetworkAccessConditionFindMatchingChildrenChildren(res gjson.Result, child NetworkAccessConditionChildrenChildren) []gjson.Result {
	var matchedResults []gjson.Result

	res.Get("children").ForEach(func(_, v gjson.Result) bool {
		childHash := NetworkAccessConditionHashChildrenChildren(child)

		jsonChild := v
		jsonChildStruct := NetworkAccessConditionChildrenChildren{
			Name: types.StringValue(jsonChild.Get("name").String()),
		...
		jsonChildHash := NetworkAccessConditionHashChildrenChildren(jsonChildStruct)
...
func (data *NetworkAccessCondition) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Children {
		var r gjson.Result
		r = NetworkAccessConditionFindMatchingChild(res.Get("response.children"), data.Children[i])[0]
		...
		for ci := range data.Children[i].Children {
			...
			var cr gjson.Result
			cr = NetworkAccessConditionFindMatchingChildrenChildren(r, data.Children[i].Children[ci])[0]
