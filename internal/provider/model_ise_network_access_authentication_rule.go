...
func NetworkAccessAuthenticationRuleHashChildrenChildren(child NetworkAccessAuthenticationRuleChildrenChildren) string {
...
func NetworkAccessAuthenticationRuleHashChildren(child NetworkAccessAuthenticationRuleChildren) string {
	// Hash the current child's fields
	parts := []string{
		child.ConditionType.ValueString(),
	...
	for _, nestedChild := range child.Children {
		nestedHashes = append(nestedHashes, NetworkAccessAuthenticationRuleHashChildrenChildren(nestedChild))
	}
...
func NetworkAccessAuthenticationRuleFindMatchingChild(res gjson.Result, child NetworkAccessAuthenticationRuleChildren) []gjson.Result {
	...
	res.ForEach(func(_, v gjson.Result) bool {
		...
		var nestedChildren []NetworkAccessAuthenticationRuleChildrenChildren
		jsonChild.Get("children").ForEach(func(_, vChild gjson.Result) bool {
			nestedChild := NetworkAccessAuthenticationRuleChildrenChildren{
		...
		jsonChildHash := NetworkAccessAuthenticationRuleHashChildren(jsonChildStruct)
...
func NetworkAccessAuthenticationRuleFindMatchingChildrenChildren(res gjson.Result, child NetworkAccessAuthenticationRuleChildrenChildren) []gjson.Result {
	var matchedResults []gjson.Result

	res.Get("children").ForEach(func(_, v gjson.Result) bool {
		childHash := NetworkAccessAuthenticationRuleHashChildrenChildren(child)

		jsonChild := v
		jsonChildStruct := NetworkAccessAuthenticationRuleChildrenChildren{
			ConditionType: types.StringValue(jsonChild.Get("conditionType").String()),
		...
		jsonChildHash := NetworkAccessAuthenticationRuleHashChildrenChildren(jsonChildStruct)
...
func (data *NetworkAccessAuthenticationRule) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Children {
		var r gjson.Result
		r = NetworkAccessAuthenticationRuleFindMatchingChild(res.Get("response.rule.condition.children"), data.Children[i])[0]
		...
		for ci := range data.Children[i].Children {
			...
			var cr gjson.Result
			cr = NetworkAccessAuthenticationRuleFindMatchingChildrenChildren(r, data.Children[i].Children[ci])[0]
