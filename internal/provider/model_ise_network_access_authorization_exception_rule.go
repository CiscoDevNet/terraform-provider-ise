...
func NetworkAccessAuthorizationExceptionRuleHashChildrenChildren(child NetworkAccessAuthorizationExceptionRuleChildrenChildren) string {
...
func NetworkAccessAuthorizationExceptionRuleHashChildren(child NetworkAccessAuthorizationExceptionRuleChildren) string {
	// Hash the current child's fields
	parts := []string{
		child.ConditionType.ValueString(),
	...
	for _, nestedChild := range child.Children {
		nestedHashes = append(nestedHashes, NetworkAccessAuthorizationExceptionRuleHashChildrenChildren(nestedChild))
	}
...
func NetworkAccessAuthorizationExceptionRuleFindMatchingChild(res gjson.Result, child NetworkAccessAuthorizationExceptionRuleChildren) []gjson.Result {
	...
	res.ForEach(func(_, v gjson.Result) bool {
		...
		childHash := NetworkAccessAuthorizationExceptionRuleHashChildren(child)

		// Compute hash for child from JSON result
		jsonChild := v
		// Build a NetworkAccessAuthenticationRuleChildren from jsonChild to compute hash
		jsonChildStruct := NetworkAccessAuthorizationExceptionRuleChildren{
		...
		var nestedChildren []NetworkAccessAuthorizationExceptionRuleChildrenChildren
		jsonChild.Get("children").ForEach(func(_, vChild gjson.Result) bool {
			nestedChild := NetworkAccessAuthorizationExceptionRuleChildrenChildren{
		...
		jsonChildHash := NetworkAccessAuthorizationExceptionRuleHashChildren(jsonChildStruct)
...
func NetworkAccessAuthorizationExceptionRuleFindMatchingChildrenChildren(res gjson.Result, child NetworkAccessAuthorizationExceptionRuleChildrenChildren) []gjson.Result {
	var matchedResults []gjson.Result

	res.Get("children").ForEach(func(_, v gjson.Result) bool {
		childHash := NetworkAccessAuthorizationExceptionRuleHashChildrenChildren(child)

		jsonChild := v
		jsonChildStruct := NetworkAccessAuthorizationExceptionRuleChildrenChildren{
			ConditionType: types.StringValue(jsonChild.Get("conditionType").String()),
		...
		jsonChildHash := NetworkAccessAuthorizationExceptionRuleHashChildrenChildren(jsonChildStruct)
...
func (data *NetworkAccessAuthorizationExceptionRule) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Children {
		var r gjson.Result
		r = NetworkAccessAuthorizationExceptionRuleFindMatchingChild(res.Get("response.rule.condition.children"), data.Children[i])[0]
		...
		for ci := range data.Children[i].Children {
			...
			var cr gjson.Result
			cr = NetworkAccessAuthorizationExceptionRuleFindMatchingChildrenChildren(r, data.Children[i].Children[ci])[0]
