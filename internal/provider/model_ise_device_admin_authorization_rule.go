...
func DeviceAdminAuthorizationRuleHashChildrenChildren(child DeviceAdminAuthorizationRuleChildrenChildren) string {
...
func DeviceAdminAuthorizationRuleHashChildren(child DeviceAdminAuthorizationRuleChildren) string {
	// Hash the current child's fields
	parts := []string{
		child.ConditionType.ValueString(),
	...
	for _, nestedChild := range child.Children {
		nestedHashes = append(nestedHashes, DeviceAdminAuthorizationRuleHashChildrenChildren(nestedChild))
	}
...
func DeviceAdminAuthorizationRuleFindMatchingChild(res gjson.Result, child DeviceAdminAuthorizationRuleChildren) []gjson.Result {
	...
	res.Get(".").ForEach(func(_, v gjson.Result) bool {
		...
		var nestedChildren []DeviceAdminAuthorizationRuleChildrenChildren
		jsonChild.Get("children").ForEach(func(_, vChild gjson.Result) bool {
			nestedChild := DeviceAdminAuthorizationRuleChildrenChildren{
		...
		jsonChildHash := DeviceAdminAuthorizationRuleHashChildren(jsonChildStruct)
...
func DeviceAdminAuthorizationRuleFindMatchingChildrenChildren(res gjson.Result, child DeviceAdminAuthorizationRuleChildrenChildren) []gjson.Result {
	var matchedResults []gjson.Result

	res.Get("children").ForEach(func(_, v gjson.Result) bool {
		childHash := DeviceAdminAuthorizationRuleHashChildrenChildren(child)

		jsonChild := v
		jsonChildStruct := DeviceAdminAuthorizationRuleChildrenChildren{
			ConditionType: types.StringValue(jsonChild.Get("conditionType").String()),
		...
		jsonChildHash := DeviceAdminAuthorizationRuleHashChildrenChildren(jsonChildStruct)
...
func (data *DeviceAdminAuthorizationRule) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Children {
		var r gjson.Result
		r = DeviceAdminAuthorizationRuleFindMatchingChild(res.Get("response.rule.condition.children"), data.Children[i])[0]
		...
		for ci := range data.Children[i].Children {
			...
			var cr gjson.Result
			cr = DeviceAdminAuthorizationRuleFindMatchingChildrenChildren(r, data.Children[i].Children[ci])[0]
