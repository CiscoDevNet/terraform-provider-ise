...
func TestAccIseEndpointCustomAttribute(t *testing.T) {
	...
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccIseEndpointCustomAttributeConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
...
func testAccIseEndpointCustomAttributeConfig_minimum() string {
...
func testAccIseEndpointCustomAttributeConfig_all() string {
	config := `resource "ise_endpoint_custom_attribute" "test" {` + "\n"
	config += `	attribute_name = "isMobile"` + "\n"
	config += `	attribute_type = "Boolean"` + "\n"
	config += `}` + "\n"
