...
func DeviceAdminPolicySetHashChildrenChildren(child DeviceAdminPolicySetChildrenChildren) string {
...
func DeviceAdminPolicySetHashChildren(child DeviceAdminPolicySetChildren) string {
	// Hash the current child's fields
	parts := []string{
		child.ConditionType.ValueString(),
	...
	for _, nestedChild := range child.Children {
		nestedHashes = append(nestedHashes, DeviceAdminPolicySetHashChildrenChildren(nestedChild))
	}
...
func DeviceAdminPolicySetFindMatchingChild(res gjson.Result, child DeviceAdminPolicySetChildren) []gjson.Result {
	...
	res.ForEach(func(_, v gjson.Result) bool {
		...
		var nestedChildren []DeviceAdminPolicySetChildrenChildren
		jsonChild.Get("children").ForEach(func(_, vChild gjson.Result) bool {
			nestedChild := DeviceAdminPolicySetChildrenChildren{
		...
		jsonChildHash := DeviceAdminPolicySetHashChildren(jsonChildStruct)
...
func DeviceAdminPolicySetFindMatchingChildrenChildren(res gjson.Result, child DeviceAdminPolicySetChildrenChildren) []gjson.Result {
	var matchedResults []gjson.Result

	res.Get("children").ForEach(func(_, v gjson.Result) bool {
		childHash := DeviceAdminPolicySetHashChildrenChildren(child)

		jsonChild := v
		jsonChildStruct := DeviceAdminPolicySetChildrenChildren{
			ConditionType: types.StringValue(jsonChild.Get("conditionType").String()),
		...
		jsonChildHash := DeviceAdminPolicySetHashChildrenChildren(jsonChildStruct)
...
func (data *DeviceAdminPolicySet) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Children {
		var r gjson.Result
		r = DeviceAdminPolicySetFindMatchingChild(res.Get("response.condition.children"), data.Children[i])[0]
		...
		for ci := range data.Children[i].Children {
			...
			var cr gjson.Result
			cr = DeviceAdminPolicySetFindMatchingChildrenChildren(r, data.Children[i].Children[ci])[0]
