...
func (data *TrustSecSecurityGroup) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get("Sgt.description"); value.Exists() && value.Type != gjson.Null && value.String() != "" {
...
func (data *TrustSecSecurityGroup) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get("Sgt.description"); value.Exists() && !data.Description.IsNull() && value.String() != "" {
		data.Description = types.StringValue(value.String())
	} else {
		data.Description = types.StringNull()
	}
