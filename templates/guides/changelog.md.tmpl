...
- Fix destroy/recreate of `ise_active_directory_join_point` during brownfield import, where ISE returns existing `groups` and `rewrite_rules` in the GET response but these attributes were not marked as `Computed`, causing Terraform to plan a destroy and recreate of the resource to reconcile the difference [#260](https://github.com/CiscoDevNet/terraform-provider-ise/issues/260)
